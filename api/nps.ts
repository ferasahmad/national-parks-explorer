...
export async function getParks(params: GetParksParams): Promise<NPSResponse<Park>> {
  return fetchNPS<NPSResponse<Park>>('/parks', params as Record<string, string | number>);
}
...
export async function getParksByCodes(parkCodes: string[]): Promise<Park[]> {
  ...
  const response = await fetchNPS<NPSResponse<Park>>('/parks', { parkCode: parkCodes.join(',') });
